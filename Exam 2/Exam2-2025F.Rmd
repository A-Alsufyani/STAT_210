...
\bigskip
\begin{center}\color{red}
{\textbf{For all tests in this exam use a significance level of $\mathbf \alpha$ = 0.02 \\unless otherwise specified}}
\end{center}
...
From the summary table of our minimal model, we extract two functions for the lines, one if the fertilizer is organic and one otherwise.
 ...
 Yield_chemical = 1.9056084 + 0.0082430 * Rainfall
...
(c) Using a backward selection procedure with a critical $\alpha$ of 0.10 and starting with the variables you selected in (b), obtain a minimal adequate model. Comment on the steps that you take.
...
\texttt{age} & \texttt{bmi} & \texttt{sbp} & \texttt{dbp} &  \texttt{hr} & \texttt{cmb} & \texttt{p.adm} &  \texttt{severity}   \\
